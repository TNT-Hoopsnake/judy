...
def load_all_data():
    ...
    for run_name in os.listdir(DATA_DIR):
        ...
        for task in config["eval"].tasks:
            if (
                matches_tag(task, metadata["task_tags"])
                and task.id in config["run"].tasks
            ):
                all_data_list[run_name]["tasks_used"].append(task)

        for dataset in config["datasets"]:
            if matches_tag(dataset, metadata["dataset_tags"]) and set(
                dataset.tasks
            ).intersection(set(config["run"].tasks)):
